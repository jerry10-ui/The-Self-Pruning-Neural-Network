...
**Candidate submission for Tredence AI Engineering Internship – 2025 Cohort**
...
### Key Implementation Detail: Normalisation

Naively summing all gate values (1.7M+ parameters) causes the sparsity loss to dwarf the classification loss. The sparsity loss is instead computed as the **mean** gate value:

```python
return total_gate_sum / total_gate_count   # always in [0, 1]
```

This keeps sparsity loss comparable in magnitude to CrossEntropy (~2.3), making λ interpretable and tunable.
...
| Sparsity threshold = 0.5 | More meaningful than 0.01 for sigmoid gates; counts gates more "off" than "on" |
